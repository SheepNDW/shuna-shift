...
describe('PageHeader', () => {
  it('renders kanji, label and title', async () => {
    ...
    expect(wrapper.get('h1.page-header__title').text()).toBe('完整班表');
  });
  ...
  it('renders the subtitle only when provided', async () => {
    ...
    expect(without.find('.page-header__subtitle').exists()).toBe(false);
    ...
    expect(withSubtitle.get('.page-header__subtitle').text()).toBe('近期排班紀錄');
  });
  ...
  it('renders the meta slot only when provided', async () => {
    ...
    expect(without.find('.page-header__meta').exists()).toBe(false);
    ...
    expect(withMeta.get('.page-header__meta').text()).toBe('正職 6 · 現役 12');
  });
});
